...
class HTDRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(HTDRoIHead, self).__init__(
        ...
        if self.with_global:
            self.glbctx_head = build_head(
                dict(
                    ...
                    # num_classes=81,
                    num_classes=self.bbox_head[0].num_classes+1,
                    loss_weight=3.0))
    ...
    def _bbox_forward(self, stage, x, rois, global_feat=None, sampling_results=None, img_metas=None):
        ...
        enhanced_extractor = self.bbox_roi_extractor[1]
        if stage == 0:
            ...
            if sampling_results:
                ...
                bbox_head = self.bbox_head[stage]
                bbox_feats = bbox_roi_extractor(x[:bbox_roi_extractor.num_inputs], rois)
                spatial_enhanced_feat = enhanced_extractor(x[:bbox_roi_extractor.num_inputs], pos_rois)

                pos_bbox_feat = torch.cat((bbox_feats[:num_pos_1], bbox_feats[num_boxs_1:num_boxs_1 + num_pos_2]),
                                          dim=0)

                if self.with_global:
                    cls_score, bbox_pred = bbox_head(bbox_feats, pos_bbox_feat, x[:bbox_roi_extractor.num_inputs], rois,
                                                     self.bbox_head[0].fc_cls, spatial_enhanced_feat, pos_rois,
                                                     global_feat)
                else:
                    cls_score, bbox_pred = bbox_head(bbox_feats, pos_bbox_feat, x[:bbox_roi_extractor.num_inputs], rois,
                                                     self.bbox_head[0].fc_cls, spatial_enhanced_feat)
                ...
                bbox_results = dict(cls_score=cls_score, bbox_pred=bbox_pred_pos_)
                # bbox_results = dict(cls_score=cls_score, bbox_pred=bbox_pred)
            else:
                # testing mode: we don't have sampling results!
                bbox_head = self.bbox_head[stage]
                bbox_feats = bbox_roi_extractor(x[:bbox_roi_extractor.num_inputs], rois)
                spatial_enhanced_feat = enhanced_extractor(x[:bbox_roi_extractor.num_inputs], rois)

                if self.with_global:
                    cls_score, bbox_pred = bbox_head(bbox_feats, bbox_feats, x[:bbox_roi_extractor.num_inputs], rois,
                                                     self.bbox_head[0].fc_cls, spatial_enhanced_feat, rois, global_feat)
                else:
                    cls_score, bbox_pred = bbox_head(bbox_feats, bbox_feats, x[:bbox_roi_extractor.num_inputs], rois,
                                                     self.bbox_head[0].fc_cls, spatial_enhanced_feat)
                bbox_results = dict(cls_score=cls_score, bbox_pred=bbox_pred)
    ...
    def forward_train(self,
        ...
        if self.with_global:
            mc_pred, global_feat = self.glbctx_head(x)
